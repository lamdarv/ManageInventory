...
const ModalAddInventory = ({ isOpen, onRequestClose }) => {
  const [nama_inventaris, setNamaInventaris] = useState("");
  ...
  const handleSubmit = async (event) => {
    ...
    const hasSpecialChars =
      specialCharsRegex.test(nama_inventaris) || specialCharsRegex.test(deskripsi);
    ...
    if (!nama_inventaris || !deskripsi || !tgl_kepemilikan || !status || !peminjam) {
      window.alert("Semua field harus diisi!");
      return;
    }

    const formattedPeminjam = peminjam.map((p) => ({
      nim: p.value.nim,
      nama: p.value.name,
    }));
    ...
    formData.append("nama", nama_inventaris);
    ...
    try {
        ...
        if (response.status === 200) {
          ...
          console.error("Failed to create inventory:", response.statusText);
        }
      } catch (error) {
        console.error(error);
      }
  ...
  const getAllMahasiswa = async () => {
    try {
        const response = await axios.get("/api/user/mahasiswa", {
            headers: {
                Authorization: `Bearer ${user.token}`,
            },
        });
        if (response.status === 200){
            ...
            const filterMahasiswa = responseData.map((user) => ({
                value: { nim: user.nim, name: user.name },
                label: `${user.nim} - ${user.name}`,
            }));

            setPeminjamOptions(filterMahasiswa);
            console.log(setPeminjamOptions);
        } else {
    ...
    //   console.log(peminjamList);
      ...
      console.error("Error get all user by role mahasiswa:", error);
    }
  };
              ...
              <div className="left">
                <div className="mb-6 w-96">
                  ...
                  <input
                    ...
                    onChange={(e) => setNamaInventaris(e.target.value)}
                    required="true"
                  />
