...
const Posts = () => {
  ...
  const options = posts.map((post) => ({
    label: post.list_peminjam.map((peminjam) => (
      <li key={peminjam}>{peminjam}</li>
    )),
    value: post.list_peminjam.map((peminjam) => (
      <li key={peminjam}>{peminjam}</li>
    )),
  }));
        ...
        <div className="md:container md:mx-auto">
          <div className="flex flex-wrap justify-center">
            {posts.map((post) => (
              <div
                key={post._id}
                className="mr-6 ml-6 mt-10 p-6 bg-white rounded-lg shadow-md w-full md:w-3/4"
              >
                <table>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td className="font-quicksand font-normal text-lg pr-6">
                      {new Date(post.tgl_kepemilikan).toLocaleDateString(
                        "id-ID"
                      )}
                    </td>
                  </tr>
